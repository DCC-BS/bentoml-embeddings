...
ValidStringList = Annotated[list[ValidString], MinLen(1), MaxLen(MAX_DOCS)]
...
class Embeddings:
    ...
    def __init__(self) -> None:
        ...
        self.encoder_model = SentenceTransformer(
            EMBEDDING_MODEL_ID, trust_remote_code=True, device=self.device
        )
    ...
    @bentoml.api()
    def encode_queries(self, queries: ValidStringList) -> np.ndarray:
        """
    ...
    def rerank(
        self,
        documents: ValidStringList = Field(
            description="List of documents to be reranked"
        ),
        query: ValidString = Field(
            description="Query string used for ranking documents"
        ),
        ...
        """
        ranked_results: RankedResults = self.ranker.rank(
            query=query, docs=documents, doc_ids=list(range(0, len(documents)))
        )
        result_json = {
            result.doc_id: (
                result.text,
    ...
    def tokenize(
        self,
        documents: ValidStringList = Field(
            description="Documents that should be tokenized"
        ),
    ) -> t.List[t.List[int]]:
